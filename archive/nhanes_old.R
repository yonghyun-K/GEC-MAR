...
theta = mean(data0$BPXSY1)

# Simulation setup of
# Wang & Kim, 2025
n = nrow(data0)
pcol = ncol(data0); # pcol = 4 is True
x = as.matrix.data.frame(data0[,-1])
summary(x)
z = cbind(log(x[,1] - 10) * 10, rbinom(n, 1, x[,2] * 0.5 + 0.25),
          x[,3] * ifelse(x[,1] < 32, 0.5, 1), x[,3] * x[,4] / 16)
colnames(z) <- colnames(x)
summary(z)

pi = 1 / (1 + exp(-(-3 - x[,1] / 10 + 0.5 * x[,2] +
                      x[,3] / 10 - 0.25 * x[,4]))) # True
mean(pi)
...
final_res <- foreach(
  ...
  .errorhandling="pass") %dopar% {
    ...
    for(cnt in 1:nrow(modelcases)){
      ...
      # OM = T; RM = T
      if(RM == T){
        Rmodel = glm(reformulate(paste0("x.", colnames(x)), response = "delta"), family = binomial,
                     data = data)
        # Rmodel = glm(reformulate(c("0", paste0("x.", 1:pcol)), response = "delta"), family = binomial,
        #              data = data)
        x_S0 = cbind(1, x_S); x0 = cbind(1, x)
      }else{
        Rmodel = glm(reformulate(paste0("z.", colnames(x)), response = "delta"), family = binomial,
                     data = data)
      ...
      if(OM == T){
        Omodel = lm(reformulate(paste0("x.", colnames(x)), response = "y"), weights = 1 / vx, data = data_S)
      }else{
        Omodel = lm(reformulate(paste0("z.", colnames(x)), response = "y"), weights = 1 / vx, data = data_S)
      }
      ...
      findphi0 = function(phi, x0, Z, delta, ..., returnw = F){
      ...
      jacphi0 = function(phi, x0, Z, delta, ..., returnw = F){
        pi_phi = drop(1 / (1 + exp(-x0 %*% phi)))
        return(-t(Z) %*% (x0 * (pi_phi * (1 - pi_phi)) ))
      }
      
      Rmodel0 = glm(reformulate(paste0("x0.", colnames(x)), response = "delta"), family = binomial,
                    data = data.frame(x0 = x0[,-1], delta))
      (nleqslv_res = nleqslv(Rmodel0$coefficients, findphi0, jac = jacphi0, x0 = x0, Z = x0, delta = delta,
                             control = list(maxit = 1e5, allowSingular = T), xscalm = "auto",
                             method = "Newton"))
      
      if(nleqslv_res$termcd != 1 & max(abs(findphi0(nleqslv_res$x, x0 = x0, Z = x0, delta = delta))) > 1e-5){
        w_S = NA
      }else{
        w_S = findphi0(nleqslv_res$x, x0 = x0, Z = x0, delta = delta, returnw = T)
      }
      w_S0 = w_S
      ...
      for(pimethod in 1:3){
        ...
        v_S = vx[Index_S];
        u_vec = matrix(-pihat * vx, ncol = 1) # EL1
        u_vec = cbind(-x * ((1 - pihat) / pihat / gprime1(1 / pihat, type = "EL", del = del) * vx), u_vec)
        u_vec_S = u_vec[Index_S,]; Uhat = colMeans(u_vec); 
        Z_S = cbind(1, x_S, u_vec_S); Zbar = c(1, colMeans(x), Uhat); Z_St = t(Z_S)
        ...
        targetftn = function(del, pihat, x, y, folds){
          ...
          performance_metrics <- sapply(folds, function(test_indices) {
            ...
            u_vec0 = cbind(-x * ((1 - pihat) / pihat / gprime1(1 / pihat, type = type, del = del) * vx), u_vec0)
        ...
        u_vec = cbind(-x * ((1 - pihat) / pihat / gprime1(1 / pihat, type = type, del = del) * vx), u_vec)
...
for(ncols in 1:Ncol){
  boxres = t(do.call(cbind, lapply(final_res1, c)))[, ((ncols - 1) * Nrow + 1) : (ncols * Nrow)] + theta
  colnames(boxres) <- row.names(final_res1[[1]])
  boxplot(boxres, main = paste(paste(c("RP"), ifelse(modelcases[ncols,], "C", "M"), sep = ":"), collapse = ", "), 
