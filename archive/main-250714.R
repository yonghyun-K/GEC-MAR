    ...
    for(cnt in 1:nrow(modelcases)){
      ...
      for(pimethod in c(1,3)){
        ...
        for(type in c("EL", "ET", "GE")){
          ...
          if (rcond(Z_St %*% (Z_S * gprime1(1 / pihat[Index_S], type = type, del = del) / d_S / v_S)) < 1e-12) {
            gammahat = rep(NA, ncol(Z_S))
          } else {
            gammahat = solve(Z_St %*% (Z_S * gprime1(1 / pihat[Index_S], type = type, del = del) / d_S / v_S),
                             Z_St %*% (y_S * gprime1(1 / pihat[Index_S], type = type, del = del) / d_S / v_S))
          }
          yhat2 = drop(cbind(1, xO, u_vec) %*% gammahat)
          if (rcond(t(hhat[Index_S,]) %*% (hhat[Index_S,] * (1 / pihat[Index_S] - 1) / pihat[Index_S])) < 1e-12) {
            gammahat = rep(NA, ncol(hhat))
          } else {
            kappa = solve(t(hhat[Index_S,]) %*% (hhat[Index_S,] * (1 / pihat[Index_S] - 1) / pihat[Index_S]),
          ...
          var_res = c(var_res, setNames(var(eta) / n, paste(type, 0, sep = "")))
          ...
          # gammahat = MASS::ginv(Z_St %*% (Z_S * gprime1(1 / pihat[Index_S], type = type, del = del) / d_S / v_S)) %*%
          #   Z_St %*% (y_S * gprime1(1 / pihat[Index_S], type = type, del = del) / d_S / v_S)
          if (rcond(Z_St %*% (Z_S * gprime1(1 / pihat[Index_S], type = type, del = del) / d_S / v_S)) < 1e-12) {
            gammahat = rep(NA, ncol(Z_S))
          } else {
          ...
          # var_res = c(var_res, setNames(sum((c(y_S * w_S + drop(Z_S %*% gammahat) * (1 - w_S),
          #                                      yhat2[!Index_S]) - sum(y_S * w_S) / n)^2) / n / (n-1), type))
          
          eta = yhat2 + drop(hhat %*% kappa)
          eta[Index_S] = eta[Index_S] + (y_S - yhat2[Index_S]) * w_S
          var_res = c(var_res, setNames(var(eta) / n, type))
          
          # var_res = c(var_res, setNames(sum(w_S * (w_S - 1) * (y_S - yhat2[Index_S])^2) / n / (n-1), type))
        }
